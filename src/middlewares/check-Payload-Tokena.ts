...
export const checkPayloadTokena = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (dateExp < new Date()) return res.status(HTTP_STATUSES.FORBIDDEN_403).send('Expired date')

    const deviceUser = await deviceRepositories.findDeviceForValid(payload.userId, payload.deviceId, payload.iat)
    if(deviceUser) return res.status(HTTP_STATUSES.FORBIDDEN_403).send('Incorrect userId or deviceId')
    req.payload = payload
    next()
}
